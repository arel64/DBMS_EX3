...
def insert_data(csv_file):
    ...
    for i, row in tqdm(df.iterrows(), total=len(df)):
        if debug_mode and i % 100 != 0:
            continue
        movie_id = int(row['id']) if pd.notna(row['id']) and str(row['id']).isdigit() else 0
        budget = int(row['budget']) if pd.notna(row['budget']) and str(row['budget']).isdigit() else 0
        homepage = row['homepage'] if pd.notna(row['homepage']) else ''
        original_language = row['original_language'] if pd.notna(row['original_language']) else ''
        original_title = row['original_title'] if pd.notna(row['original_title']) else ''
        ...
        revenue = int(row['revenue']) if pd.notna(row['revenue']) and str(row['revenue']).isdigit() else 0
        ...
        vote_count = int(row['vote_count']) if pd.notna(row['vote_count']) and str(row['vote_count']).isdigit() else 0
        ...
        for g in genres_data:
            cursor.execute("INSERT IGNORE INTO genres(genre_id,name) VALUES(%s,%s)", (g['id'], g['name']))
            cursor.execute("INSERT IGNORE INTO movie_genres(movie_id,genre_id) VALUES(%s,%s)", (movie_id, g['id']))
        companies_data = []
        ...
        for c in companies_data:
            cursor.execute("INSERT IGNORE INTO production_companies(company_id,name) VALUES(%s,%s)", (c['id'], c['name']))
            cursor.execute("INSERT IGNORE INTO movie_companies(movie_id,company_id) VALUES(%s,%s)", (movie_id, c['id']))
    cnx.commit()
    cursor.close()
    cnx.close()
